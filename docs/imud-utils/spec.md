...
| `imu.bus.integrity` | A register whose value cannot change — the part's identity, the one `probe()` compares — is read 2000 times and must come back unchanged every time. A mismatch at the real address is a corrupted transfer, not a wrong part. Graded as a rate: any bad read WARNs, 0.5% or more FAILs. It sits before the timing checks deliberately, because every figure below it is downstream of the bus being able to carry a byte. Two defects reached the bench disguised as something else before this existed — a `TIMESTAMP0` read 1.58 s high that presented as a timestamp-chain fault, and a factory-trim register reading differently across two `init()`s that presented as a state-dependent `init()`. The mag equivalent is `mag.bus.integrity`. |
...
| `spin.magnitude` | Mean \|B\| while turning is 25–65 µT, the same band `mag.field_magnitude` uses at rest. Measured across the whole turn, so unlike the resting figure it cannot be flattered by one lucky orientation. |
