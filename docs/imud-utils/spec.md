...
| `gyro.bias` | **INFO, not graded.** The gyro rest bias removed from every sample the rotation and spin phases integrate, and where it was measured: the rest window's mean when the passive phase ran, otherwise a short window taken before the phase's first prompt. Both integrating windows stay open until the operator signals and are mostly not turning, so an uncorrected integral carries the bias for the whole window rather than for the turn — enough on its own to put a good gyro outside `.scale`'s band. WARNs instead, and subtracts nothing, when that window collected nothing or was not still (per-axis sigma over 0.1 rad/s): a hand turn's mean is not a bias, and removing it would corrupt every angle below. |
