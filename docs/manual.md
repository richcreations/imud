...
| `mag_reject_gauss` | double | `0.05` | Strong-anomaly cutoff: reject a magnetometer measurement whose post-calibration residual exceeds this value (Gauss). Guards against nearby iron/magnets. The residual measured is the one the update fuses — the horizontal (heading) component under `mag_yaw_only`, the full field vector otherwise, so the dip a swing-circle calibration cannot measure never gates a healthy heading. Fine-grained consistency is handled internally by χ² innovation gates that self-scale with filter confidence, so keep this a coarse threshold (~10% of the Earth field). |
