...
| `fifo_wm` | int | `64` | FIFO watermark in sample-sets: how deep the chip buffers before raising its interrupt. This sets **both buffer depth and sample latency** — with `int_grace`, the reader waits `fifo_wm + int_grace` sample periods, so 64 at 833 Hz batches about 77 ms. Earlier releases documented this key as having *no* effect on latency, citing a bench run that measured the same 33 ms residence at every setting. That measurement was real and the conclusion drawn from it was wrong: the reader then used a flat 10 ms fallback, which drained before the watermark could assert — and because a level watermark deasserts as soon as a drain empties the FIFO, the interrupt was suppressed entirely at nine of the ten ISM330DHCX rates. The key was inert because its own safety net pre-empted it. On the ST parts a watermark of 8 or more also batches the chip timestamp into the FIFO (§1.1); at 32 and above that costs 1.6% of the word budget, so the same watermark holds about 63 sample-sets rather than 64. |
...
| `mmc5983ma` | MEMSIC MMC5983MA | Magnetometer | 0x30 | BCM 27 · pin 13 | **yes** — mode 0, 10 MHz | Primary reference mag. 18-bit, SET/RESET coil. On SPI a CTRL0 write also lands in CTRL1, so the driver rewrites CTRL1 after every one. |
...
| Heading drifts steadily while pitch and roll stay put | The magnetometer is uncalibrated, so every mag sample is marked invalid, the filter's mag update never runs and heading is dead-reckoned from the gyro — measured on a static bench, 220° in 24 minutes while pitch and roll held to a tenth of a degree. `imud-status` says so explicitly under Attitude; the fix is `imud-cal mag`. |
