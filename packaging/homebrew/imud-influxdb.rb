...
class ImudInfluxdb < Formula
  desc "InfluxDB bridge for the imud IMU daemon"
  ...
  def install
    ...
    system "make", "imud-influxdb"
    system "make", "install-influxdb", "DESTDIR=#{buildpath}/stage"
  ...
  service do
    run [opt_bin/"imud-influxdb", "--config", etc/"imud/imud-influxdb.conf"]
    keep_alive true
    log_path var/"log/imud-influxdb.log"
    error_log_path var/"log/imud-influxdb.log"
  end

  test do
    assert_match "imud-influxdb", shell_output("#{bin}/imud-influxdb --version")
    assert_match "--config", shell_output("#{bin}/imud-influxdb --help")
  end
end
