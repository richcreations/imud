...
static int bank_sel(const imud_bus_t *bus, uint8_t bank)
{
    return bus_reg_write(bus, REG_BANK_SEL, (uint8_t)(bank << 4));
}
...
static int icm_probe(const imud_bus_t *bus)
{
    ...
    uint8_t who;
    if (bus_reg_read(bus, B0_WHO_AM_I, &who) < 0) {
...
static int icm_reset(const imud_bus_t *bus)
{
    ...
    if (bus_reg_write(bus, B0_PWR_MGMT_1, 0x80) < 0) return -1;

    for (int i = 0; i < 100; i++) {
        usleep(1000);
        uint8_t val;
        if (bus_reg_read(bus, B0_PWR_MGMT_1, &val) < 0) return -1;
...
static int icm_init(const imud_bus_t *bus, const imu_cfg_t *cfg)
{
    ...
    if (bus_reg_write(bus, B0_PWR_MGMT_1, CLKSEL_AUTO) < 0) return -1;
    usleep(5000);

    /* Enable accel and gyro (clear disable bits). */
    if (bus_reg_write(bus, B0_PWR_MGMT_2, 0x00) < 0) return -1;

    /* ── Bank 2: sample rate + full-scale + DLPF ─────────────────────────── */
    if (bank_sel(bus, 2) < 0) return -1;

    /* Gyro: sample rate divider, then config (FCHOICE=1, DLPFCFG=3 ~51Hz NBW). */
    if (bus_reg_write(bus, B2_GYRO_SMPLRT_DIV, gdiv) < 0) return -1;
    if (bus_reg_write(bus, B2_GYRO_CONFIG_1,
                  (uint8_t)(0x01 | (3 << 3) | gfs)) < 0) return -1;

    /* Accel: 12-bit sample rate divider, then config (FCHOICE=1, DLPFCFG=3 ~50Hz NBW). */
    if (bus_reg_write(bus, B2_ACCEL_SMPLRT_1, (uint8_t)(adiv >> 8)) < 0) return -1;
    if (bus_reg_write(bus, B2_ACCEL_SMPLRT_2, (uint8_t)(adiv & 0xFF)) < 0) return -1;
    if (bus_reg_write(bus, B2_ACCEL_CONFIG,
    ...
    if (bus_reg_write(bus, B0_USER_CTRL, USER_CTRL_FIFO_EN) < 0) return -1;

    /* Reset FIFO: assert then deassert. */
    if (bus_reg_write(bus, B0_FIFO_RST, 0x1F) < 0) return -1;
    usleep(100);
    if (bus_reg_write(bus, B0_FIFO_RST, 0x00) < 0) return -1;

    /* Stream mode: overwrite oldest when full. */
    if (bus_reg_write(bus, B0_FIFO_MODE, 0x00) < 0) return -1;
                    ...
                    imu_sample_t *buf, int max, int *n_out)
{
    *n_out = 0;
    ...
    uint8_t cnt[2];
    if (bus_burst_read(bus, B0_FIFO_COUNTH, cnt, 2) < 0) return -1;
    ...
    uint8_t tmp[2];
    if (bus_burst_read(bus, B0_TEMP_OUT_H, tmp, 2) == 0) {
