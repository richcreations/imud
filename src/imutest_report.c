...
/*
 * Snap a value that would render as a negative zero to a positive one.
 *
 * printf turns -0.00004 into "-0.0000", and a report cell reading "-0.00" has
 * already been reported once as a defect -- it looks like a measured sign on a
 * quantity that has none.  `eps` is half the last printed place, so the caller
 * states it beside the format it is about to use.
 */
static double nz(double v, double eps)
{
    return fabs(v) < eps ? 0.0 : v;
}
                 ...
                 char *errbuf, size_t errbufsz)
{
    ...
    fprintf(f, "\n\n");
    /* Attach, not paste.  A report runs to a few hundred lines and several
     * are usually compared side by side, which a thread of pasted walls of
     * text makes painful; attaching also keeps the file intact, where an
     * email-client round trip has silently dropped one.  Saying so here is
     * what the reader acts on — the file arrives before any documentation
     * does. */
    fprintf(f, "<!-- generated by imud-imutest %s; attach this whole FILE to "
               "a GitHub issue (the add-files button), do not paste it -->\n\n",
            r->imud_version);
     ...
     */
    /*
     * The sample stream itself, before any statistic is taken of it.  A mean
     * and a sigma cannot tell zeros from a stuck value from a plausible number
     * in the wrong field; eight consecutive rows can, at a glance.  Both
     * windows print together so the reader compares them rather than scrolling
     * between sections — the interesting failures are the ones where the part
     * measures correctly at rest and not while it is being driven.
     */
    if (w->n_rest_rows > 0 || w->n_turn_rows > 0) {
        const struct { const char *title; int n; const imt_sample_row_t *rows; }
            win[2] = { { "at rest",               w->n_rest_rows, w->rest_rows },
                       { "during the first turn", w->n_turn_rows, w->turn_rows } };
        fprintf(f, "### 5.2b Sample stream as the driver delivered it\n\n");
        fprintf(f, "```\n");
        for (int s = 0; s < 2; s++) {
            if (win[s].n == 0) continue;
            fprintf(f, "%s\n", win[s].title);
            fprintf(f, "      seq        aX        aY        aZ"
                       "        gX        gY        gZ     temp\n");
            for (int i = 0; i < win[s].n; i++) {
                const imt_sample_row_t *q = &win[s].rows[i];
                fprintf(f, "%9u %9.4f %9.4f %9.4f %9.5f %9.5f %9.5f %8.2f\n",
                        q->seq,
                        nz(q->accel[0], 5e-5), nz(q->accel[1], 5e-5),
                        nz(q->accel[2], 5e-5),
                        nz(q->gyro[0], 5e-6), nz(q->gyro[1], 5e-6),
                        nz(q->gyro[2], 5e-6), nz(q->temp_c, 5e-3));
            }
            if (s == 0 && win[1].n) fprintf(f, "\n");
        }
        fprintf(f, "```\n\naccel m/s^2, gyro rad/s, temp degC.\n\n");
    }
